...
export const usePokemonStore = defineStore('pokemonStore', {
  state: () => ({
    ...
    totalFavoritePokemon: localStorage.getItem('favoritePokemon')
      ? JSON.parse(localStorage.getItem('favoritePokemon') || '[]').length
      : 0,
  }),
  getters: {},
  actions: {
    saveFavoritePokemon(pokemon: FavouritePokemon) {
      ...
      const favoritePokemonList = storedFavorites
    ...
    removeFavoritePokemon(name: string) {
      const storedFavorites = localStorage.getItem('favoritePokemon')
      if (storedFavorites) {
        const favoritePokemonList = JSON.parse(storedFavorites).filter(
    ...
    isFavoritePokemon(name: string): boolean {
      ...
      if (storedFavorites) {
        ...
        return favoritePokemonList.some(
          (pokemon: FavouritePokemon) => pokemon.name === name
        )
      }
      return false
    ...
    loadFavoritePokemon(page: number, pageSize: number) {
      const storedFavorites = localStorage.getItem('favoritePokemon')
      this.totalFavoritePokemon = storedFavorites
        ? JSON.parse(storedFavorites).length
        : 0
      const favoritePokemonList = JSON.parse(storedFavorites || '[]')
